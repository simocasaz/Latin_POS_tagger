...
UNIVERSAL_FEATURES = {
    "Abbr", "Aspect", "Case", "Degree", "InflClass", "InflClass[nominal]", "Mood", "Number", "Person", "Tense", "VerbForm", "Voice"
}
...
def load_conllu(file):
    ...
    while True:
        ...
        if not line:
            ...
            def process_word(word,parseIDX):
                ...
                if word.parent is None:
                    ...
                    if head < 0 or head > len(ud.words) - sentence_start:
                        raise UDError("HEAD '{}' points outside of the sentence".format(_encode(parseIDX)))
                    if head:
                        parent = ud.words[sentence_start + head - 1]
                        word.parent = "remapping"
                        process_word(parent,parseIDX)
                        word.parent = parent
                        parseIDX = parseIDX+1
            parseIDX = 0
            for word in ud.words[sentence_start:]:
                process_word(word,parseIDX)
   ...
   #R         try:
   #R             head_id = int(columns[HEAD])
   #R         except:
   #R             raise UDError("Cannot parse HEAD '{}'".format(_encode(columns[HEAD])))
   #R         if head_id < 0:
   #R             raise UDError("HEAD cannot be negative")
...
def main():
    ...
    if not args.verbose and not args.counts:
        ...
        print("-----------+-----------+-----------+-----------+-----------")
        for metric in["Tokens", "Sentences", "Words", "UPOS", "UFeats" ,"Lemmas"]:
            if args.counts:
